...
class HomeViewModel(
    ...
    fun loadHistory(force: Boolean = false) {
        if (!force && historyShared.history.value.isNotEmpty()) return
        screenModelScope.launch {
            mutableState.update { it.copy(loading = true) }
            recordRepo
                .getRecords()
                .onSuccess { data ->
                    historyShared.setHistory(data)
                }.onFailure { e ->
                    if (!e.isUnauthorized()) {
                        appEvents.emit(
                            AppEvent.ShowSnackbar(e.message ?: "Terjadi kesalahan"),
                        )
                    }
                }
            mutableState.update { it.copy(loading = false) }
    ...
    fun switchStats(value: Boolean) {
        mutableState.update { it.copy(switcher = value) }
    }
}
